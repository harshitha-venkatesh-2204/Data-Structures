...
public class Huffman {
   ...
   public void calculateHuffmanCodes() {
        // For each leaf entry with frequency greater than 0, calculate its Huffman code.
        for (int i = 0; i < SIZE; i++) {
            ...
            if (entry.freq > 0) {
                ...
                // Traverse up to the root and prepend the code to build the Huffman code.
                while (current.parent != null) {
                    code = current.code + code;
                    current = current.parent;
                }

                // Assign the calculated Huffman code to the entry.
                entry.code = code;
            }
        }
    }
